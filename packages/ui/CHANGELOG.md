...
* add a "Promote selected jobs" bulk action to the Delayed tab's selection toolbar, using the new `promoteJobs` mutation instead of requiring one-by-one promotion
...
* add the Bull Horizon logo as the dashboard favicon and header icon
* rename the in-app header wordmark and page `<title>` from "Bull monitor" to "Bull Horizon"
