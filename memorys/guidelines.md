...
- **CI existe desde 2026-08-11** (`.github/workflows/ci.yml`: lint + jest + bootstrap + build). Ainda assim, validar localmente antes de entregar ao Ops — o CI é rede de segurança, não substituto. `npx eslint . --ext .ts,.tsx` está **verde no repo inteiro**; qualquer erro novo é da mudança em curso, não drift herdado.
- **Validar mudanças de pacote de servidor contra o `root` local**: `npm run build` em `packages/root` e symlink `packages/<adapter>/node_modules/@bull-horizon/root` → `packages/root`. Sem isso o `tsc` do adapter valida contra a versão publicada no npm.
- **Trabalhando em worktree** (`.worktrees/*`): não há `node_modules` lá. Copiar do checkout principal com `cp -al` (hard links, rápido e barato) — **não** symlinkar o diretório inteiro: o `tsc` com `declaration: true` resolve tipos por um caminho fora do projeto e falha com `TS2742 ... cannot be named without a reference to ...`.
