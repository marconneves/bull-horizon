...
- **Sem `workspaces` no `package.json` raiz nem hoisting automático**: `npm install` dentro de um pacote (ex: `packages/express`) baixa `@bull-monitor/root` **publicado no npm** (versão antiga), não o código local em `packages/root/src`. Para validar `tsc`/build de um adapter contra mudanças locais em `packages/root`, é preciso: 1) `npm run build` em `packages/root` (gera `dist/`); 2) sobrescrever `node_modules/@bull-monitor/root` do adapter com um symlink para `packages/root` (`ln -s <abs-path>/packages/root <abs-path>/packages/<adapter>/node_modules/@bull-monitor/root`, usando caminhos absolutos — `cd` persiste entre comandos do Bash tool e paths relativos erram silenciosamente). Sem isso, `npx tsc --noEmit` valida contra a versão errada e pode passar ou falhar por motivos enganosos.
