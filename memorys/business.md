...
- O dashboard tem **4 telas**: `jobs` (lista da fila ativa), `metrics` (gráficos da fila ativa), `overview` (grid de todas as filas com distribuição de status) e `history` (throughput agregado de todas as filas + tabela por fila). `metrics` e `history` só aparecem com o metrics collector configurado; a navegação cross-queue vive na sidebar, e o botão do AppBar alterna jobs ↔ métricas **da fila aberta**.
...
- **Retenção default de métricas: 3 dias** (`maxMetrics: 4320` a 1 min). Seletores de janela na UI vão até `3d` de propósito — oferecer `90d` sobre uma série de 3 dias seria um gráfico vazio com rótulo confiante.
