...
- **bull-monitor**: dashboard/UI padrão para monitorar filas [Bull](https://github.com/OptimalBits/bull) e [BullMQ](https://github.com/taskforcesh/bullmq) (bibliotecas de job queue para Node.js sobre Redis).
...
- **Adapter**: pacote que integra o core (`@bull-monitor/root`) a um framework HTTP específico (Express, Koa, Hapi, Fastify) como middleware GraphQL.
- **CLI**: forma de uso standalone (`@bull-monitor/cli`) que sobe um servidor Express embutido sem exigir integração em app existente.

## 2. Regras de Negócio Core (O que a aplicação restringe logica ou operacionalmente)

- O produto é **distribuído como biblioteca/middleware**, não como serviço hospedado: o usuário final integra `@bull-monitor/<framework>` na própria aplicação Node.js (ou usa o CLI standalone).
