...
- O dashboard tem **3 telas**, todas na sidebar, agrupadas por escopo: **Active queue** → `jobs` (lista da fila ativa); **All queues** → `overview` (grid de todas as filas com distribuição de status) e `history` (throughput agregado + tabela por fila). `history` só aparece com o metrics collector configurado.
