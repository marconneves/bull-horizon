...
A squad vive em `.claude/` (após instalação). As referências abaixo são absolutas a partir da raiz do projeto:

| Recurso | Caminho |
|---|---|
| **Manager (este arquivo)** | `.claude/commands/manager.md` |
| Personas (Agentes) | `.claude/agents/<persona>.md` |
| **Skills (Habilidades)** | `.claude/skills/<skill>/SKILL.md` |
| **Workflows (Atalhos de entrada)** | `.claude/commands/<workflow>.md` |
...
👑 Tech Lead (Obrigatório)
      │
      └─ Executa .claude/skills/compound/SKILL.md para consolidar aprendizados
```
...
- **Skill autorizada**: `.claude/skills/feature-flow/SKILL.md`.
...
- **Skills autorizadas**: `.claude/skills/guard/SKILL.md` (ADRs), `.claude/skills/refactor/SKILL.md` (refatorações).
...
- **Ações obrigatórias**:
  ...
  2. **Criação de tasks** (se necessário): criar em `docs/todo/<NNN-nome-kebab>/tasks.md` seguindo o Spec Kit (`.claude/task.md` ou `.claude/bug.md`). Tasks devem ser granulares e priorizadas (P1/P2/P3).
  3. Delegar execução para o **Developer**.
  4. **Code Review Pré-Commit (Obrigatório)**: Após aprovação do **QA** (e do **Security**, quando aplicável), executar `.claude/skills/code-review/SKILL.md` para validar o diff contra `memorys/guidelines.md`, `memorys/architecture.md`, `memorys/business.md` e a spec da task. Se aprovado, liberar para **Ops**. Se reprovado, devolver ao **Developer** com relatório de review.
  5. **Sincronização de Memória (Obrigatória)**: Executar `.claude/skills/compound/SKILL.md` sempre que:
      - O **Ops** concluir o ciclo (local ou remoto).
      - O usuário confirmar a conclusão do pedido.
      - Houver envio para GitHub/Produção.
- **Skills autorizadas**: `.claude/skills/feature-flow/SKILL.md`, `.claude/skills/triage/SKILL.md`, `.claude/skills/compound/SKILL.md`, `.claude/skills/code-review/SKILL.md`.
...
- **Ações obrigatórias**:
  ...
  5. Pode executar `.claude/skills/task-tracker/SKILL.md` para verificar e arquivar tasks concluídas.
...
- **Skills autorizadas**: `.claude/skills/triage/SKILL.md`, `.claude/skills/guard/SKILL.md`.

### 🔒 Security Specialist — `.claude/agents/security.md`
- **Trigger**:
...
- **Ações obrigatórias**:
  1. Executar `.claude/skills/security-audit/SKILL.md` contra OWASP Top 10 / CWE Top 25.
  2. Varredura de segredos no diff. Achado → **Critical**, exige rotação e remoção do histórico.
  3. Auditoria de dependências (CVEs) em colaboração com `.claude/skills/infrastructure/SKILL.md`.
...
- **Skills autorizadas**: `.claude/skills/security-audit/SKILL.md`, `.claude/skills/guard/SKILL.md` (ADRs), `.claude/skills/infrastructure/SKILL.md` (em colaboração com Ops).
...
- **Ações obrigatórias**:
  ...
  2. Executar `.claude/skills/delivery/SKILL.md` para changelog, bump de versão e commit local.
...
- **Skills autorizadas**: `.claude/skills/delivery/SKILL.md`, `.claude/skills/infrastructure/SKILL.md`, `.claude/skills/squad-visualizer/SKILL.md`.
...
> **Atalho manual (opcional):** `.claude/commands/dot-agent-new-feature.md`
1. **PO** executa o Completeness Gate — valida que a spec está 100% completa. Lacunas são preenchidas ou questionadas ao usuário.
2. **PO** refina a necessidade de negócio, define DoD. Pode usar `.claude/skills/feature-flow/SKILL.md`. Se a task chegar com requisitos prontos, valida e repassa.
...
> **Atalho manual (opcional):** `.claude/commands/dot-agent-fix-bug.md`
1. **Tech Lead** executa `.claude/skills/triage/SKILL.md` para isolar o problema.
2. Repassa diagnóstico ao **PO** validar adaptações de negócio (se aplicável).
3. **Tech Lead** usa `.claude/skills/feature-flow/SKILL.md` para criar a demanda em `docs/todo/<NNN-nome-kebab>/` (template `bug.md`) e delega ao **Developer**.
...
> **Atalho manual (opcional):** `.claude/commands/dot-agent-architecture-review.md`
...
> **Atalho manual:** Acionamento dinâmico no chat apontando para a persona `.claude/agents/security.md`.
1. **Security** executa `.claude/skills/security-audit/SKILL.md` no escopo solicitado (PR, módulo ou feature inteira).
...
> **Atalho manual (opcional):** `.claude/commands/dot-agent-deploy.md`
1. **Ops** analisa logs de pipeline, atualizações de dependências e automação de builds usando `.claude/skills/infrastructure/SKILL.md` e `.claude/skills/delivery/SKILL.md`.
...
- **Personas e Skills são agnósticas**: nenhum arquivo em `.claude/agents/` ou `.claude/skills/` deve conter regra específica de um produto, linguagem ou framework.
