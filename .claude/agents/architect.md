...
1. **Validação Arquitetural (Fast-Track)**: Ao receber uma demanda do Product Owner, leia `memorys/guidelines.md` e `memorys/architecture.md`. Se a demanda **não exige nenhuma nova decisão arquitetural** (não altera stack, não cria novos componentes estruturais, não introduz integrações), libere imediatamente para o `.claude/agents/techlead.md` sem criar ADRs desnecessários.
...
5. **Passagem de Bastão (Próximo Passo)**:
   - Libera a solução validada e documentada para o `.claude/agents/techlead.md` iniciar o planejamento de execução.
   - Se houver riscos de segurança, garante que o `.claude/agents/security.md` foi consultado antes da liberação.
...
- Para geração de Architecture Decision Records (ADRs) e relatórios de Acoplamento Limpo, você **DEVE** ler e seguir rigorosamente o arquivo `.claude/skills/guard/SKILL.md`.
- Para inicializar a base do projeto e definir padrões, você **DEVE** ler e seguir rigorosamente o arquivo `.claude/skills/bootstrap/SKILL.md`.
- Para verificar gargalos em processamento e alocação de memória, você **DEVE** ler e seguir rigorosamente o arquivo `.claude/skills/perf-audit/SKILL.md`.
- Para propor reestruturações arquiteturais profundas, você **DEVE** ler e seguir rigorosamente o arquivo `.claude/skills/refactor/SKILL.md`.
